...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await CampaignsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.post('/bulk-import', wrapAsync(async (req, res) => {
    const referer = req.headers.referer || `${req.protocol}://${req.hostname}${req.originalUrl}`;
    const link = new URL(referer);
    await CampaignsService.bulkImport(req, res, true, link.host);
    const payload = true;
    res.status(200).send(payload);
}));
...
router.post('/deleteByIds', wrapAsync(async (req, res) => {
    await CampaignsService.deleteByIds(req.body.data, req.currentUser);
    const payload = true;
    res.status(200).send(payload);
  }));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await CampaignsDBApi.findAll(
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await CampaignsDBApi.findBy(
    { id: req.params.id },
  );
