...
const CardFeedbacks = ({
  ...
  return (
    <div className={'p-4'}>
      ...
      >
        {!loading && feedbacks.map((item, index) => (
          <li
            key={item.id}
          ...
          >
              <Link href={`/feedbacks/feedbacks-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.name}
              </Link>
              ...
              <div className='ml-auto'>
                <ListActionsPopover
                  onDelete={onDelete}
                  itemId={item.id}
                  pathEdit={`/feedbacks/feedbacks-edit/?id=${item.id}`}
                  pathView={`/feedbacks/feedbacks-view/?id=${item.id}`}
