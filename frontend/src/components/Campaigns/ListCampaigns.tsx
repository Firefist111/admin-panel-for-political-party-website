...
const ListCampaigns = ({ campaigns, loading, onDelete, currentPage, numPages, onPageChange }: Props) => {
                  ...
                  <div key={item.id}>
                    <CardBox hasTable isList className={'rounded shadow-none'}>
                        <div className={`flex rounded dark:bg-dark-900 border items-center overflow-hidden`}>
                          <Link
                              href={`/campaigns/campaigns-view/?id=${item.id}`}
                              className={
                          ...
                          >
                            ...
                            <div className={'flex-1 px-3'}>
                                ...
                                <p className={'line-clamp-2'}>{ item.events }</p>
                            </div>
                            ...
                            <ListActionsPopover
                              onDelete={onDelete}
                              itemId={item.id}
                              pathEdit={`/campaigns/campaigns-edit/?id=${item.id}`}
                              pathView={`/campaigns/campaigns-view/?id=${item.id}`}
