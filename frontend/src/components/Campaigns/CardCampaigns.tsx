    ...
    <div className={'p-4'}>
      ...
      >
        {!loading && campaigns.map((item, index) => (
          ...
          >
              <Link href={`/campaigns/campaigns-view/?id=${item.id}`} className='text-lg font-bold leading-6 line-clamp-1'>
                  {item.title}
              </Link>
            ...
            <dl className='divide-y dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                    <dt className='text-gray-500 dark:text-dark-600'>Events</dt>
                    <dd className='flex items-start gap-x-2'>
                        <div className='font-medium line-clamp-4'>
                            { item.events }
                        </div>
