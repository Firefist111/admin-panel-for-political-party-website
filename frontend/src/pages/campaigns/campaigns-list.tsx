...
const CampaignsTablesPage = () => {
  ...
  const addFilter = () => {
    ...
    const getCampaignsCSV = async () => {
        const response = await axios({url: '/campaigns?filetype=csv', method: 'GET',responseType: 'blob'});
  ...
  return (
    <>
      ...
      <SectionMain>
        <SectionTitleLineWithButton icon={mdiChartTimelineVariant} title="Campaigns" main>
        {''}
        </SectionTitleLineWithButton>
        <CardBox   className='mb-6' cardBoxClassName='flex flex-wrap'>
          <BaseButton className={'mr-3'} href={'/campaigns/campaigns-new'} color='info' label='New Item' />
          <BaseButton
              className={'mr-3'}
              color='info'
              label='Filter'
              onClick={addFilter}
          />
          <BaseButton className={'mr-3'} color='info' label='Download CSV' onClick={getCampaignsCSV} />
            <BaseButton
              color='info'
              label='Upload CSV'
